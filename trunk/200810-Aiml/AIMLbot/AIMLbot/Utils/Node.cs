...
namespace AIMLbot.Utils
{
    ...
    public class Node
    {
        ...
        public string evaluate(string path, SubQuery query, Request request, MatchState matchstate, StringBuilder wildcard)
        {
            ...
            string starContainedPattern = string.Empty;
            foreach(string childKey in this.children.Keys)
            {
                if (childKey.Contains("*") == true)
                {
                    starContainedPattern = childKey;
                    break;
                }
            }

            if (starContainedPattern.Length > 0)
            {
                // o.k. look for the path in the child node denoted by "*"
                Node childNode = (Node)this.children[starContainedPattern];

                // add the next word to the wildcard match 
                StringBuilder newWildcard = new StringBuilder();
                string wordSplited = splitPath[0];

                string found = FindWordsForStar(starContainedPattern, wordSplited);
                if (found.Length > 0)
                {
                    this.storeWildCard(found, newWildcard);
        ...
        private static string FindWordsForStar(string starContainedPattern, string wordSplited)
        {
            string pattern = WildcardToRegex(starContainedPattern);
            Regex regex = new Regex(pattern);
            Match match = regex.Match(wordSplited);
            ...
            /*
            int starIndex = starContainedPattern.IndexOf("*"); // 0
            if(starIndex > 0)
                wordSplited = wordSplited.Remove(0, starIndex);
            int rightCount = wordSplited.Length - (starContainedPattern.Length - starIndex - 1);
            if (rightCount > 0)
                wordSplited = wordSplited.Substring(0, rightCount);
                                
            this.storeWildCard(wordSplited, newWildcard);
             * */
        }
